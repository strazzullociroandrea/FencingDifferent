...
function distribuisciGiocatori(numeroGironi, listaGiocatori) {
  ...
  for (let i = 0; i < numeroGiocatori; i++) {
    ...
    const indiceGirone = i % numeroGironi; // Calcola l'indice del girone circolarmente
...
/**
 * Funzione roundRobin per generare gli incontri
 * @param {*} lunghezza
 * @returns
 */
...
/**
 * Funzione per visualizzare in tabella gli incontri
 * @param {*} lista
 * @param {*} div
 * @param {*} roundRobin
 */
...
export const renderGironi = (nomeTorneo, data) => {
  recuperaGironi(nomeTorneo, data)
    .then((response) => {
      ...
      //---> 2 - accoppiamento atleti per id assalto
